...
const CANONICAL_WEEK_ORDER={1:["u1","l1","u2","l2","arms"],2:["u2","l2","u1","l1","arms"],3:["u1","l1","u2","l2","arms"],4:["u2","l2","u1","l1","arms"],5:["u1","l1","u2","l2","arms"],6:["u2","l2","u1","l1","arms"],7:["u1","l1","u2","l2","arms"]};
...
for(const w of Object.keys(CANONICAL_WEEK_ORDER)){
  const actual=CANONICAL_WEEK_ORDER[w].map(key=>PROGRAM[w].days[DAYS.find(d=>PROGRAM[w].days[d]===key)]);
  if(actual.join(",")!==CANONICAL_WEEK_ORDER[w].join(",")) throw new Error(`Ordem inválida na semana ${w}`);
}
if(METHODS["7"].u1!=="SSSSSSSS"||METHODS["7"].l1!=="SSSSSSSSS"||METHODS["7"].u2!=="CSCCSCCC"||METHODS["7"].l2!=="SCCCSCSCS") throw new Error("Semana 7: método inválido");
...
function renderCycleCompleted(){
 layout(`<section class="hero cycle-complete">
   <div class="eyebrow">Ciclo ${state.cycle} · ${APP_VERSION}</div>
...
function startWorkout(week,key,date){
  week=Math.max(1,Math.min(7,Number(week)||1));
  if(!PROGRAM[week]||!PROGRAM[week].days) throw new Error("Semana inválida");
  if(!PROGRAM[week].days[1] && !PROGRAM[week].days[2]) throw new Error("Programação inválida");
  workout={week,key,date,index:0};
  route="overview";
  renderOverview();
  bindNav();
}
...
function historyOccurrenceRows(key,index,cycleId){
 const rows=[];
 for(let week=1;week<=7;week++){
  ...
  const e=exercisesFor(week,key)[index];
  rows.push({week,logs,method:e?.method||"straight"});
 }
 return rows;
}
function historyPerformance(row){
 if(!row.logs.length)return `<span class="history-empty">—</span>`;
 if(row.method==="cluster"){
  const groups={};
  row.logs.forEach(l=>(groups[l.number]??=[]).push(l));
  return Object.keys(groups).sort((a,b)=>Number(a)-Number(b)).map(n=>{
   const blocks=groups[n].sort((a,b)=>(a.block||0)-(b.block||0)).map((l,i)=>`B${i+1}: ${l.weight||"—"} kg × ${l.reps||"—"}`).join(" · ");
   return `<div class="history-set"><span>Cluster ${n}</span><strong>${blocks}</strong></div>`;
  }).join("");
 }
...
function renderHistory(){
 ...
 let cards="";
 exs.forEach((e,i)=>{
  const rows=historyOccurrenceRows(selected,i,cycle.id);
  const has=rows.some(r=>r.logs.length);
  cards+=`<section class="history-ex-card ${has?"has-data":""}">
   <div class="history-ex-head"><div class="history-ex-number">${i+1}</div><div class="history-ex-main"><div class="h2">${esc(e.name)}</div><div class="muted">Comparação sessão a sessão · mesma posição do exercício · cargas e reps separadas por método</div></div></div>
   <div class="history-weeks">${rows.map((r,idx)=>`<div class="history-week ${r.logs.length?"filled":""}"><div class="history-week-label">${idx===0?"1ª sessão":"Próxima sessão"} · Semana ${r.week}<br><small>${r.method==="cluster"?"CLUSTER":"STRAIGHT"}</small></div><div class="history-week-value">${historyPerformance(r)}</div></div>`).join("")}</div>
  </section>`;
 });
 layout(`<section class="hero history-hero"><div class="eyebrow">Histórico</div><div class="h1">Evolução por treino</div><p class="muted">Compare cada exercício pela sequência em que o mesmo treino acontece: uma sessão do ${LABEL[selected]} contra a próxima sessão do ${LABEL[selected]}, dentro do mesmo ciclo.</p>${cycleTabs?`<div class="history-cycles">${cycleTabs}</div>`:""}</section><div class="history-tabs">${tabs}</div><div class="history-summary"><strong>${LABEL[selected]}</strong><span>Ciclo ${cycle.id} · sessão → próxima sessão</span></div><div class="history-list">${cards}</div>`);
