const CACHE="mr-saizen-v41";
const VERSION="41";
const ASSETS=["./?v=41","./index.html?v=41","./app-v41.js?v=41","./styles.css?v=41","./manifest.webmanifest?v=41","./icon.svg?v=41"];
...
self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;const u=new URL(e.request.url);if(u.origin!==location.origin)return;
 ...
 if(u.pathname.endsWith("app-v41.js")||u.pathname.endsWith("styles.css")||u.pathname.endsWith("manifest.webmanifest")||u.pathname.endsWith("icon.svg")){e.respondWith(fetch(e.request,{cache:"no-store"}).then(r=>{const c=r.clone();caches.open(CACHE).then(x=>x.put(e.request,c));return r;}).catch(()=>caches.match(e.request)));return;}
